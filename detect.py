...
Usage - sources:16406
    $ python detect.py --weights last.pt --source ../datasets/video/B3.avi                               # webcam
                                                     img.jpg                         # image
                                                     vid.mp4                         # video
                                                     screen                          # screenshot
                                                     path/                           # directory
...
def run(
    ...
    if is_url and is_file:
        source = check_file(source)  # 寻找源测试文件 或者下载文件
    ...
    for path, im, im0s, vid_cap, s in dataset: #path是图片的绝对路径 im0s是原图，im是缩放后的
        ...
        with dt[2]: # 此时pred有十位，前八位为xyxyxyxy，然后是概率，类别
            _, pred = non_max_suppression(pred,nclr, conf_thres, iou_thres, classes, True, max_det=max_det)
...
def parse_opt():
    ...
    parser.add_argument('--nclr', type=int, default=3, help='nof colors')
