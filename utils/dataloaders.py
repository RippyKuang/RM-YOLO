...
def img2label_paths(img_paths):
    return [x.split('.')[0]+".txt" for x in img_paths]
...
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
        ...
        try:
            ...
            cache, exists = self.cache_labels(cache_path, prefix,non), False  # run cache ops
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix='',non=False):
        # Cache dataset labels, check images and read shapes
        x = {}  # dict
        nm, nf, ne, nc, msgs = 0, 0, 0, 0, []  # number missing, found, empty, corrupt, messages
        desc = f'{prefix}Scanning {path.parent / path.stem}...'
        with Pool(8) as pool:
            pbar = tqdm(pool.imap(verify_image_label, zip(self.im_files, self.label_files, repeat(prefix),repeat(non))),
                        desc=desc,
    ...
    def __getitem__(self, index):
        ...
        if self.augment:
            ...
            if random.random() < self.pcbg and nl>0 :
...
def convert_non(lb):
    new_lb = []
    for l in lb :
        if l[0] not in ['2','5','8','11','14','17','20','23','32']:
            l[0] = int(l[0]) -int(l[0])//3
            new_lb.append(l)
    return np.array(new_lb, dtype=np.float32)


def verify_image_label(args):
    # Verify one image-label pair
    im_file, lb_file, prefix, non = args
