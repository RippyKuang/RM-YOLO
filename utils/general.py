...
def labels_to_image_weights(labels, nc=80, class_weights=np.ones(80)):
    ...
    class_counts = np.array([np.bincount(x[:, 0].astype(int), minlength=nc) for i,x in enumerate(labels)]) #每张图片含有的物体，形状是(128,80),第二维表示物体出现的次数，没有就是0
...
def non_max_suppression(
    ...
    xc = prediction[:, :,8+nclr:].amax(-1) > conf_thres  # candidates
    ...
    for xi, _x in enumerate(prediction):  # image index, image inference
        ...
        conf, j = _x[:, 8+nclr:].max(1, keepdim=True)  #函数会返回两个tensor，第一个tensor是每行的最大值；第二个tensor是每行最大值的索引,即置信度最高的类别
        if nclr!=0:
            _ , i = _x[:, 8:8+nclr].max(1, keepdim=True)  #函数会返回两个tensor，第一个tensor是每行的最大值；第二个tensor是每行最大值的索引,即置信度最高的类别
        else:
            i=j
        thres_ = conf.view(-1) > conf_thres
        x = torch.cat((box, conf, (j*nclr+i).float()), 1)[thres_] #0-3是box，4是置信度，5是类别
        x_ep = torch.cat((_x[:, :8], conf, (j*nclr+i).float()),1)[thres_]
        # 从这里开始x的格式都是xyxy
