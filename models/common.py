...
class CoordConv(nn.Module):
    ...
    def forward(self, ins_feat):
        ...
        return self.act(self.bn(self.conv(ins_feat)))
    def forward_fuse(self, x):
...
class dwConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        self.pw = nn.Conv2d(c1, c2, 1, 1,groups=g,dilation=d, bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
    def forward(self, x):
...
class DFL(nn.Module):
    ...
    def __init__(self, c1=32):
        ...
        self.conv.weight.data[:] = nn.Parameter(torch.arange(1-c1//2,1+c1//2, dtype=torch.float).view(1, c1, 1, 1)) # / 120.0
        self.c1 = c1
        # self.bn = nn.BatchNorm2d(4)

    def forward(self, xi):
        b, x, y ,_ = xi.shape  # batch, channels, anchors
        
        return self.conv(xi.view(b, x, y, 8, self.c1 ).softmax(4).view(b*x*y,8,self.c1,1).transpose(2, 1)).view(b, x, y, 8)
        # return self.conv(x.view(b, self.c1, 4, a).softmax(1)).view(b, 4, a)

class DecoupledHead(nn.Module):
   
    def __init__(self, nc, nclr, reg_max, l):
        super().__init__()
        self.nclr = nclr
        self.nc = nc
        ...
        self.clso  = l[1]
        self.rego  = make_divisible(l[3],8)

        self.cls_merge = dwConv(l[0],self.clso, 3)
        self.cls_conv = Conv(self.clso, self.clso, 3, 1, 1)
        self.cls_preds = nn.Conv2d(self.clso, (self.nc), 1)
      
        self.reg_merge = dwConv(l[0],self.rego, 3, 1, 1,g=4)
        self.reg_conv = Conv(self.rego, self.rego, 3, 1, 1,g=4)
        self.reg_preds = nn.Conv2d(self.rego, (8)*self.reg_max, 1, 1,groups= 4)

        
        if nclr != 0:
            self.clro = l[2]
            self.clr_merge = dwConv(l[0],self.clro, 3)
            self.clr_conv = Conv(self.clro, self.clro, 3, 1, 1)
            self.clr_preds = nn.Conv2d(self.clro, (self.nclr), 1)
        else:
            self.clro =0
