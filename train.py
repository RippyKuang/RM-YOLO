...
#python .\train.py --cfg yolov5n.yaml --data data/detaset.yaml --hyp data/hyps/high.yaml --epoch 51 --batch-size 16   --noautoanchor --cache disk --image-weights
...
from utils.autoanchor import check_anchors,xyxyxyxy2xywh
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:  #pretrained
        ...
        model = Model(cfg or ckpt['model'].yaml, nclr,ch=3, nc=nc, anchors=hyp.get('anchors'),regmax=reg_max).to(device)  # create
        ...
        model = Model(cfg, ch=3, nc=nc, anchors=hyp.get('anchors'),regmax=reg_max).to(device)  # create
    amp = check_amp(model)  # check AMP

    # Freeze
    freeze = [f'model.{x}.' for x in (freeze if len(freeze) > 1 else range(freeze[0]))]  # layers to freeze
    for k, v in model.named_parameters(): #k是模型参数的键 v是具体的参数
    ...
    # Image size
    gs = max(int(model.stride.max()), 32)  # grid size (max stride)   最大的下采样倍率
    imgsz = check_img_size(opt.imgsz, gs, floor=gs * 2)  # verify imgsz is gs-multiple  确保图片大小是gs的整数倍

    # Batch size 让电脑自己选择batchsize
    if RANK == -1 and batch_size == -1:  # single-GPU only, estimate best batch size
        batch_size = check_train_batch_size(model, imgsz, amp)
        loggers.on_params_update({'batch_size': batch_size})

    # Optimizer
    nbs = 64 #名义上的batchsize
    accumulate = max(round(nbs / batch_size), 1)  # 需要积累几次实际的bs才能达到名义上的bs
    hyp['weight_decay'] *= batch_size * accumulate / nbs  # 缩放权值衰减，batch_size > nbs 时 增大衰减速度
    optimizer = smart_optimizer(model, opt.optimizer, hyp['lr0'], hyp['momentum'], hyp['weight_decay'])  #把模型的参数划分到三个不同的组中

    # Scheduler
    if opt.cos_lr:
        lf = one_cycle(1, hyp['lrf'], epochs)  # cosine 1->hyp['lrf'] lr周期变化？ 懒得看了
    else:
        lf = lambda x: (1 - x / epochs) * (1.0 - hyp['lrf']) + hyp['lrf']  # lr线性变化
    scheduler = lr_scheduler.LambdaLR(optimizer, lr_lambda=lf)  # plot_lr_scheduler(optimizer, scheduler, epochs)

    # EMA
    ema = ModelEMA(model) if RANK in {-1, 0} else None #指数移动平均 感觉有点像低通滤波
    ...
    compute_loss = ComputeLoss(model,regmax=reg_max)  # init loss class
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        dataset.pcbg = dataset.pcbg - 1/epochs if dataset.pcbg > 1/epochs else 0

        mloss = torch.zeros(4, device=device)  # mean losses
        if RANK != -1:
            train_loader.sampler.set_epoch(epoch)
        pbar = enumerate(train_loader)
        LOGGER.info(('\n' + '%11s' * 8) % ('Epoch', 'GPU_mem', 'box_loss', 'cls_loss', 'pts_loss', 'total_loss','Instances', 'Size'))
        if RANK in {-1, 0}:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/high.yaml', help='hyperparameters path')
    ...
    parser.add_argument('--non', action='store_true')
    parser.add_argument('--negative-path', nargs='+', default=['../static_sun09_database'],
                        type=str)
    parser.add_argument('--regmax', type=int, default=1)
